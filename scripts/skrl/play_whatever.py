...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, experiment_cfg: dict):
    ...
    target_joint_to_test = 0

    while simulation_app.is_running():
        start_time = time.time()

        if(time.time_ns() - t >= 5e+9):
            # print("LOOP HZ: ", hz)
            # print("TARGET HZ: ", 1/dt)
            # print("AVERAGE SLEEP TIME: ", total_sleep_time/hz)
            # print("===============================")

            target_joint_to_test += 1
            target_joint_to_test = target_joint_to_test % 12
        ...
        with torch.inference_mode():
            # agent stepping
            # obs = torch.tensor([
            #         [-0.1439, -0.0079, -0.7990, 0.0569, 0.4794, -0.6401, 0.1656, 0.6259, -0.6787, 0.2455, 0.2357, -0.6988,
