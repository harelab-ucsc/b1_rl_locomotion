...
BASE_OBS_SIZE = 12+3+12
OBS_SIZE = BASE_OBS_SIZE + 2  # base obs + 2 one-hot task bits (stand=10, sit=01)
ACT_SIZE = 12
...
class StudentModel(nn.Module):
    ...
    @staticmethod
    def obs(base_obs: torch.Tensor, task_id: int) -> np.ndarray:
        task_spec = [1, 0] if task_id < 2 else [0, 1]  # stand=10, sit=01
        return torch.cat([base_obs, task_spec])
